...
for cong in reno cubic westwood vegas; do
	for min_rto in 1000; do
		for qsize in 4; do
			for i in 1 2 3; do
				...
				python3 topology.py --dir $dir --t $time --maxq $qsize --min_rto $min_rto --disable_attacker true --cong $cong
				...
				for interburst_period in 0.4 0.45 0.5 0.55 0.6 0.65 0.7 0.75 0.8 0.85 0.9 0.95 1.0 1.05 1.2 1.4 1.6 1.8 2.0 3.0 4.0 5.0; do			
				    ...
				    python3 topology.py --dir $dir --t $time --maxq $qsize --burst_period $interburst_period --min_rto $min_rto --cong $cong
				done
			done
		done
	done
	...
	./findThroughputs.sh
	python3 cleanData.py --cong $cong
done
